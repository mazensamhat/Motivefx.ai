...
export function MarketWorkspace({
  ...
  useEffect(() => {
    ...
    if (!path || !backendUserId || !backendConnected) {
      setFeed([]);
      return;
    }
  ...
  }, [slug, backendUserId, backendConnected]);
  ...
  return (
    <div className="space-y-6">
      ...
      {!backendConnected && (
        <section className="app-upgrade-banner border-amber-500/40">
          <p className="text-sm text-slate-300">
            FastAPI backend is not connected. Run <code>uvicorn main:app --port 8001</code> in{" "}
            <code>backend/</code>.
          </p>
        </section>
      )}
      ...
      <section className="app-panel">
        ...
        <p className="mt-1 text-sm text-slate-400">
          {feed.length > 0 ? "Live feed from MotiveFX backend." : "Sample signals (backend feed or plan required)."}
        </p>
