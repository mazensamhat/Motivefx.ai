...
import { Calculator, Shield, Sparkles, Users, X, Zap } from "lucide-react";
import { ACQUISITION_CHANNELS, getAcquisitionChannel } from "../lib/acquisition";
import {
  COHORT_ORDER,
...
export interface ProfileSetupData {
  cohort: CohortId;
  /** Always prefer_not_to_say from onboarding — gender is not collected here. */
  sex: string;
  gender: string;
...
/**
 * First-run personalization: cohort (+ optional acquisition channel only).
 * Sex/gender are never shown in onboarding (App Store 5.1.1(v)).
 */
    ...
    <div className="gen-setup-overlay" onClick={onDismiss}>
      <div className="gen-setup-modal glass-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <header className="gen-setup-header">
          ...
          <div>
            <h2 className="gen-setup-title">Personalize Your Terminal</h2>
            <p className="gen-setup-sub">
              Select your experience profile — copy and pricing hooks adapt to your cohort. We do not
              ask for gender during setup.
            </p>
          </div>
        </header>
